...
class ManagementEnvironment:
    def __init__(self, root, OBJECT_WIDTH: int, OBJECT_HEIGHT: int, OBJECT_SPEED: int, ROOT_WIDTH: int,
                 ROOT_HEIGHT: int, player_color: str, score_total: int):
        self.root = root
...
# box of ammo
    def check_ammo_collision_player(self):
        # check if ammo is in collision with player
        for box_ammo_item in self.box_ammo_list:
            if isinstance(box_ammo_item, BoxAmmo):
    ...
    def check_to_create_box_ammo(self):
        # if not box ammo in screen and if less then x box ammo total
        if not self.box_ammo_list and self.number_of_box_ammo <= 20:
            # if less then x box ammo
            if self.number_of_ammo <= 3:
                self.create_box_ammo()
            # if less then x bullet
...
# enemy
    def random_position_of_enemy(self):
        # random position between 4 points
    ...
    def check_to_create_enemy(self):
        # less then 100 enemy and speed of creation
        if self.name_of_enemy <= 100:
            if self.name_of_enemy <= 10:
                self.call_create_enemy_several_time(1)
            elif self.name_of_enemy > 10:
                self.call_create_enemy_several_time(2)
            elif self.name_of_enemy > 30:
                self.call_create_enemy_several_time(3)
            elif self.name_of_enemy > 70:
                self.call_create_enemy_several_time(4)
        # more then 100 enemy, win
        else:
            self.player_win = True
            self.sound.play_win_game()

    def call_create_enemy_several_time(self, i: int):
        # number of creation enemy
        for _ in range(i):
            self.create_enemy()

    def create_enemy(self):
        self.name_of_enemy += 1
        # random position
        position_x, position_y = self.random_position_of_enemy()
        choice = self.random_enemy_type()
        self.enemy_list.append(Enemy(self.root, "enemy_" + str(choice), ("enemy" + str(self.name_of_enemy)),
    ...
    def create_bullet(self):
        ...
        if self.number_of_ammo >= 1:
            ...
            self.bullet_list.append(Bullet(self.root, "bullet", ("bullet" + str(self.name_of_bullet)), position_x,
                                           position_y, 20, 20, self.OBJECT_SPEED, self.position_environment,
                                           direction))
